    ...
    for port in open_ports:
        ...
        if port in streaming_ports['rtsp']:
            for path in stream_paths['rtsp']:
                ...
                thread = threading.Thread(target=check_stream_with_details, args=(url,))
                thread.daemon = True
                threads.append(thread)
                thread.start()
                found_streams = True
...
def scan_ip(ip):
    ...
    if open_ports:
        camera_found = check_if_camera(ip, open_ports)
        if not camera_found:
            ...
            if choice != "y":
                ...
                return
        check_login_pages(ip, open_ports)
        fingerprint_camera(ip, open_ports)
        test_default_passwords(ip, open_ports)
        detect_live_streams(ip, open_ports)
    else:
        print("\n[❌] No open ports found. Likely no camera here.")
    print("\n[✅] Scan Completed!")

def main():
    parser = argparse.ArgumentParser(description="CamXploit - Camera Exploitation & Exposure Scanner")
    parser.add_argument('-i', type=str, help='TXT file containing IP addresses')
    ...
    if args.i:
        ...
        ips = [input(f"{G}[+] {C}Enter IP address: {W}").strip()]
    ...
    for ip in ips:
        if validate_ip(ip):
            ...
            print(f"{R}[!] Skipping invalid IP: {ip}{W}")
