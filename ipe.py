...
# Define the output file names (filenames only)
...
def extract_data_from_sources(sources, output_dir):
    ...
    Saves the results to the specified output_dir.
    """
...
def main():
    if len(sys.argv) < 2:
        print("Usage 1 (Folder): python3 script.py -i <folder_path>")
        print("Usage 2 (Single File): python3 script.py <file_path>")
        sys.exit(1)
    ...
    if sys.argv[1] == '-i':
        ...
        # Output goes to the folder provided
    ...
    # Check if input is a single file
        ...
        # Output goes to the same directory as the file
